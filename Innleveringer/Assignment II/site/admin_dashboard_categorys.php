        ...
        if(isset($_POST['delete'])) {
            if (isset($_POST['confirm'])) {
                ...
                try {
                    ...
                    // FOREIGN KEY restrictions, set category to 1 = Uncategorized
                    if (!$success) {
                        $query =    "UPDATE articles
            				        SET category='1'
            				        WHERE category =  $current_category_id";
                        $result = $db_server -> query($query) or die('Query failed 1.1: ' . $db_server -> error);

                        $query = "DELETE FROM categorys WHERE category_id =  $current_category_id";
                        $success = $db_server -> query($query) or die('Query failed 1.2: ' . $db_server -> error);
                    }
                } catch (Exception $e) {
                    echo $e->getMessage();
                }
